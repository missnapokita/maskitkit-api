...
public class HeroGuideLoader {
	...
	private static void bindHero(
		...
		/*
		 * Main hero banner only:
		 * load the base hero artwork directly from skins.json.
		 */
		loadSkinBanner(
				activity,
				id,
	...
	private static String findSkinBanner(
		...
		JSONArray heroes = (JSONArray) root;

		String wantedId = normalize(heroId);
		String wantedName = normalize(heroName);
		...
		for (int i = 0; i < heroes.length(); i++) {
			...
			String currentId =
					normalize(hero.optString("id"));

			String currentName =
					normalize(hero.optString("name"));
			...
			/*
			 * First choice: base skin whose ID ends with 001.
			 */
			for (int j = 0; j < skins.length(); j++) {
				...
				String skinId =
						normalize(skin.optString("id"));

				String image =
						safe(skin.optString("image"));

				if (skinId.endsWith("001")
						&& isHttpUrl(image)) {
					return image;
				}
			}
			...
			/*
			 * Second choice: Backup / Default / Basic.
			 */
			for (int j = 0; j < skins.length(); j++) {
			...
			/*
			 * Final fallback: first valid skin image.
			 */
			for (int j = 0; j < skins.length(); j++) {
