...
public class HeroGuideLoader {
	...
	private static void bindHero(
		...
		/*
		 * Directly load skins.json using the same AsyncTask pattern
		 * used by Home1Fragment. This only replaces the MAIN hero banner.
		 */
		loadSkinBanner(
				activity,
				id,
			...
			protected void onPostExecute(
				...
				if (imageUrl != null &&
					...
					if (banner != null) {
						...
						/*
						 * Keep the image top-aligned even when the XML
						 * still uses a normal ImageView.
						 */
						applyTopCrop(banner);
	...
	private static String findHeroSkinImage(
			String response,
			String heroId,
		...
		for (int i = 0; i < heroes.length(); i++) {
			...
			boolean matches =
					(!wantedId.isEmpty() &&
					 wantedId.equals(currentId))
					||
					(!wantedName.isEmpty() &&
					 wantedName.equals(currentName));
			...
			/*
			 * 1. Exact base entry: hero ID + _001
			 */
			...
			for (int j = 0; j < skins.length(); j++) {
				...
				String image =
						safe(
								skin.optString("image")
						);

				if (skinId.equals(expectedBaseId) &&
					isHttpUrl(image)) {
			...
			for (int j = 0; j < skins.length(); j++) {
				...
				if ((skinName.equals("backup") ||
					 skinName.equals("default") ||
					 skinName.equals("basic")) &&
					isHttpUrl(image)) {
	...
	private static void applyTopCrop(
		...
		/*
		 * TopCropImageView handles the real top crop.
		 * For a normal ImageView this keeps CENTER_CROP as fallback.
		 */
