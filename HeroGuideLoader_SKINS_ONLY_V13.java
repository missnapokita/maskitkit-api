...
public class HeroGuideLoader {
	...
	private static final String SKINS_JSON_URL =
	...
	private static void bindHero(
		...
		/*
		 * MAIN HERO HEADER:
		 * Do not load the current API / MLBB Hub backdrop.
		 * Show a local placeholder, then replace it using skins.json only.
		 */
	...
	private static void loadSkinBanner(
			final Activity activity,
			final String heroId,
			final String heroName) {
		...
		new AsyncTask<Void, Void, String>() {
			...
			protected String doInBackground(Void... params) {
				...
				try {
					URL url = new URL(SKINS_JSON_URL);
			...
			protected void onPostExecute(String imageUrl) {
				...
				if (safe(imageUrl).isEmpty()) {
					...
					android.widget.Toast.makeText(
							activity,
							"No skins.json banner found for "
									+ valueOr(heroName, heroId),
	...
	private static String findSkinBanner(
			String json,
			String heroId,
			String heroName) throws Exception {
		...
		/*
		 * ACTUAL skins.json rule:
		 * Base hero entries have IDs with NO numeric suffix.
		 *
		 * Examples:
		 *   aamon   = base hero entry
		 *   aamon01 = numbered skin, ignore
		 */
		for (int i = 0; i < entries.length(); i++) {
			...
			String itemName =
					normalize(
							valueOr(
									item.optString("hero_name"),
									item.optString("name")
							)
					);

			if (hasNumericSuffix(rawId)) {
				continue;
			}

			boolean matches =
					(!wantedId.isEmpty()
							&& wantedId.equals(itemId))
					||
					(!wantedName.isEmpty()
							&& wantedName.equals(itemName));
			...
			/*
			 * Compatibility fallback if a future version nests skins.
			 */
			JSONArray nested =
					item.optJSONArray("skins");
			...
			if (nested != null) {
				...
				for (int j = 0; j < nested.length(); j++) {
					...
					String nestedId =
							safe(skin.optString("id"));

					if (hasNumericSuffix(nestedId)
							&& !normalize(nestedId)
							.endsWith("001")) {
						continue;
					}
		...
		/*
		 * Name-only fallback for records where id naming differs.
		 */
		for (int i = 0; i < entries.length(); i++) {
	...
	private static String firstValidImage(
			JSONObject object,
			String... keys) {
	...
	private static boolean hasNumericSuffix(
			String value) {
